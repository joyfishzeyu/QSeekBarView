...
+ (NSData *)resetSizeOfImageData:(UIImage *)sourceImage maxSize:(NSInteger)maxSize {
     ...
     */
    //思路：使用二分法搜索
    finallImageData = [UIImage halfFuntion:compressionQualityArr image:newImage sourceData:finallImageData maxSize:maxSize];
    ...
    while (finallImageData.length == 0) {
        ...
        UIImage *image = [UIImage newSizeImage:defaultSize
                                      image:[UIImage imageWithData:UIImageJPEGRepresentation(newImage,[[compressionQualityArr lastObject] floatValue])]];
        finallImageData = [UIImage halfFuntion:compressionQualityArr image:image sourceData:UIImageJPEGRepresentation(image,1.0) maxSize:maxSize];
    }
    return finallImageData;
}
...
+(UIImage*)GetImageResizing:(UIImage *)image model:(UIImageResizingMode)model{
    return [image resizableImageWithCapInsets:UIEdgeInsetsZero resizingMode:model];
}
